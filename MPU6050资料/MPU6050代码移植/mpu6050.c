...
{ 
  ...
  while(len)
  {
    //if(len==1)*buf=IIC_recvbyte(0);//读数据,发送nACK 
    //else *buf=IIC_recvbyte(1);		//读数据,发送ACK  
    if(len==1)*buf=IIC_recvbyte(1);//读数据,发送nACK 
    else *buf=IIC_recvbyte(0);		//读数据,发送ACK  
...
u8 MPU_Read_Byte(u8 reg)
{
  ...
  res=IIC_recvbyte(1);//读取数据,发送nACK 
  IIC_stop();			//产生一个停止条件 
  return res;		
}
